...
- 已实现的统一 CLI 入口：`ogi landing build`
- 已接线代码：`cli/main.py` + `cli/landing_builder.py`
- 未接线到统一 CLI 的能力：`growth-site`、`competitor-report`
- 仍存在的历史模块：`cli/page_builder.py`，当前职责是页面清单/竞品抓取实验，不是站点页渲染器
...
- CLI 入口 smoke test
- `landing build` 帮助信息与参数校验
- A/B/C/Article 四类页面渲染
- FAQ 非法输入失败路径
